...
/**
 * 读取配置顺序
 * 1. LINK 优先读取 NEXT_PUBLIC_LINK，确保部署环境可以锁定规范域名
 * 2. 其它配置优先读取 NotionConfig 表
 * 3. 再读取 blog.config.js / 或各个主题的 CONFIG 文件
 * @param {*} key ； 参数名
...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  // 站点域名必须能由部署环境锁定，避免 Notion 中的旧 LINK 覆盖
  // canonical、sitemap、robots、RSS 等公开地址。
  if (
    key === 'LINK' &&
    typeof process.env.NEXT_PUBLIC_LINK === 'string' &&
    process.env.NEXT_PUBLIC_LINK.trim()
  ) {
    return convertVal(process.env.NEXT_PUBLIC_LINK.trim())
  }
