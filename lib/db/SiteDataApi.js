...
export async function resolvePostProps({
  ...
  if (post && post.id && !post?.blockMap) {
    try {
      ...
      post.blockMap = normalizePostBlockMap(rawBlockMap)
    } catch (e) {
      console.warn('[resolvePostProps] fetchNotionPageBlocks failed:', post.id, e)
    }
  }

  // 兼容 getPost 等路径直接返回 blockMap 的情况，统一做一次归一化
  if (post?.blockMap) {
    post.blockMap = normalizePostBlockMap(post.blockMap)
  }
...
function normalizePostBlockMap(rawBlockMap) {
  if (!rawBlockMap) return rawBlockMap
